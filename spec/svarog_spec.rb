...
RSpec.describe Svarog do
  context 'noncallable thing passed to run' do
    ...
    it "raises NonCallablePassedToRun" do
      expect { ServiceObjectWithoutCall.new.call }.to raise_error(::Svarog::CallMethodNotImplemented)
    end
  end
  ...
  context 'noncallable thing passed to run' do
    ...
    it "raises NonCallablePassedToRun" do
      expect { ServiceObjectWithUncallableThing.new.call }.to raise_error(::Svarog::NonCallablePassedToRun)
    end
  end
  ...
  context 'unimplemented method' do
    ...
    it "raises MethodNotImplemented" do
      expect { ServiceObjectWithMissingMethod.new.call }.to raise_error(::Svarog::MethodNotImplemented)
    end

    it "returns message in exception" do
      begin
        ServiceObjectWithMissingMethod.new.call
      rescue ::Svarog::MethodNotImplemented => e
        expect(e.message).to eq("Slavic god Svarog is not happy because you didn't implement method")
      end
    end
  end
  ...
  context 'non wrapped value' do
    ...
    it "raises WrongDataPassed" do
      expect { ServiceObjectWithNonWrappedReturn.new.call }.to raise_error(::Svarog::WrongDataPassed)
    end

    it "returns message in exception" do
      begin
        ServiceObjectWithNonWrappedReturn.new.call
      rescue ::Svarog::WrongDataPassed => e
        expect(e.message).to eq('Slavic god Svarog is not happy with your passed data')
      end
    end
  end
  ...
  context 'first method failed' do
    ...
    it "returns result which is instance of Result" do
      result = ServiceObjectWithFailure.new.call
      expect(result).to be_an_instance_of(Svarog::Result)
    end

    it "doesn't call second method" do
      service = ServiceObjectWithFailure.new
      expect(service).to_not receive(:second_method)
      service.call
    end

    it "returns Result instance" do
      expect(ServiceObjectWithFailure.new.call).to be_an_instance_of(Svarog::Result)
    end
  end
